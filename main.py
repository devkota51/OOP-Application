...
class LoginWindow:
    ...
    def login(self):
        ...
        users = {'admin': 'password'}  # Simple user system for demo
...
class TodoApp:
    def __init__(self, master):
        ...
        # Create GUI elements
        self.task_label1 = tk.Label(master, text="To-Do Application",font="arial", foreground="blue")

        self.task_label = tk.Label(master, text="Task:", foreground="blue")
        self.task_entry = tk.Entry(master, width=30)

        self.label_label = tk.Label(master, text="Label:", foreground="blue")
        self.label_entry = tk.Entry(master, width=30)

        self.due_date_label = tk.Label(master, text="Due Date (YYYY-MM-DD):", foreground="blue")
        self.due_date_entry = tk.Entry(master, width=15)



        self.add_button = tk.Button(master, text="Add Task", command=self.add_task, bg="red", foreground="blue")
        self.listbox = tk.Listbox(master, selectmode=tk.SINGLE, width=100, height=10)
        self.edit_button = tk.Button(master, text="Edit Task", command=self.edit_task, bg="green", foreground="white")
        self.complete_button = tk.Button(master, text="Complete Task", command=self.complete_task, bg="yellow", foreground="blue")
        self.delete_button = tk.Button(master, text="Delete Task", command=self.delete_task, bg="maroon", foreground="white")
        self.save_button = tk.Button(master, text="Save Tasks", command=self.save_tasks, bg="skyblue", foreground="red")
        self.logout_button = tk.Button(master,text="Log Out", command=self.logout, bg="white", foreground="black")


        # Place GUI elements
        self.task_label1.grid(row=0, column=1, padx=15, pady=10)
        self.task_label.grid(row=1, column=0, padx=10, pady=5, sticky=tk.E)
        self.task_entry.grid(row=1, column=1, padx=10, pady=5, sticky=tk.W)
        self.label_label.grid(row=2, column=0, padx=10, pady=5, sticky=tk.E)
        self.label_entry.grid(row=2, column=1, padx=10, pady=5, sticky=tk.W)
        self.due_date_label.grid(row=3, column=0, padx=10, pady=5, sticky=tk.E)
        self.due_date_entry.grid(row=3, column=1, padx=10, pady=5, sticky=tk.W)
        ...
        self.logout_button.grid(row=8, column=2, pady=10)
    ...
    def edit_task(self):
        ...
        if selected_index:
            ...
            if new_description is not None and new_label is not None and new_due_date is not None :
        ...
        for i, task in enumerate(self.tasks, start=1):
            ...
            if completed:
                self.listbox.insert(tk.END,
                                    f"{i}. [Done] {description} ({label}) - Due: {due_date} ")
                self.listbox.itemconfig(i - 1, {'fg': 'gray'})
            else:
                self.listbox.insert(tk.END, f"{i}. {description} ({label}) - Due: {due_date} ")
